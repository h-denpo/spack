...
class Xsdk(BundlePackage):
    ...
    depends_on('hypre@2.18.2~internal-superlu+superlu-dist+shared', when='@0.5.0')
    ...
    depends_on('petsc@develop+trilinos+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64',
               when='@develop')
    depends_on('petsc@3.12.1+trilinos+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64',
               when='@0.5.0')
